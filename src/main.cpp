...
unsigned int nModifierInterval = 0.3 * nOneHour; // time to elapse before new modifier is computed
...
const string strMessageMagic = "LevoCoin Signed Message:\n";
...
{
    int64_t nSubsidy = 266 * COIN;
	
	if (nHeight == 1)
		return 90000000 * COIN;
    
	// 4 years of mining rewards, approximately
	if (nHeight > 420480)
        return nFees;
...
{
    int64_t nRewardCoinYear = 12 * CENT;
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("LevoCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in LevoCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // LevoCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "LevoCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1473386819; 
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 5666358; // !fTestNet ? 1575379 : 46534;
		
	   if (fTestNet)
			block.nNonce = 2281847;
       ...
       if (true && (block.GetHash() != hashGenesisBlock)) {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x359040c1eb3bf181232cd162f37ffbad9bbff142a81192913d94d7834aea60b3"));
...
unsigned char pchMessageStart[4] = { 0xb7, 0xe8, 0xe4, 0xcc };
