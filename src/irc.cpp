...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #LevoCoinTEST2\r");
            Send(hSocket, "WHO #LevoCoinTEST2\r");
        } else {
            // randomly join #LevoCoin00-#LevoCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #LevoCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #LevoCoin%02d\r", channel_number).c_str());
        }
