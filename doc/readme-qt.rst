LevoCoin-qt: Qt4 GUI for LevoCoin
===============================
...
Alternatively, install Qt Creator and open the `LevoCoin-qt.pro` file.

An executable named `LevoCoin-qt` will be built.
...
A warning for people using the *static binary* version of LevoCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of LevoCoin is linked against libdb5.3.
...
installed causes LevoCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
